...
import com.wyj.voice.databinding.LayoutTitleBarBinding
import com.wyj.voice.manager.ActivityManager
...
class TitleBar @JvmOverloads constructor(
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        mBinding.llBackLayer.setOnClickListener {
    ...
    private fun setAttrs(attrs: AttributeSet?, array: TypedArray) {
        ...
        for (i in 0 until count) {
            when (val attr = array.getIndex(i)) {
                ...
                R.styleable.TitleBar_leftIconVisible -> {
                    val show = array.getBoolean(attr, true)
                    val visible = if (show) VISIBLE else GONE
